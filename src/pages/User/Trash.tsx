...
import { useArchiveMailMutation, useGetArchivedMailByMailboxIdQuery, useGetMailboxInfoQuery, useReadMailMutation, useStarThreadBulkMutation, useUnReadMailMutation, useUnstarThreadBulkMutation, useUntrashMailMutation } from "../../redux/dashboardApi/user/mail/mailApi";
...
export default function Trash() {
 ...
 const { data: mailbox } = useGetMailboxInfoQuery();
     ...
     const { data, isLoading } = useGetArchivedMailByMailboxIdQuery(
       { mailboxId: mailboxId!, page: currentPage, folder: "trash" }, 
       { skip: !mailboxId }
     );
     ...
     const [untrashMailMutation] = useUntrashMailMutation();
  ...
  const untrashSelectedEmails = () => {
    selectedEmails.forEach((id) => {
      untrashMailMutation({ threadId: id });
    });
    setSelectedEmails(new Set());
  };
        ...
        {selectedEmails.size > 0 && (
          <div className="mb-4 flex flex-wrap gap-x-10 items-center gap-3 text-sm text-gray-700 bg-gray-50 p-4 rounded-md border border-gray-200">
            ...
            <div className="flex flex-wrap items-center gap-2 ">
              ...
              <button onClick={untrashSelectedEmails} className="flex items-center gap-1 gap-x-2 px-3 py-2 bg-white border border-[#0000001A] rounded-lg transition text-gray-700">
                <span>
